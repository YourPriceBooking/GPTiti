...
const PLANS: Plan[] = [
  {
    ...
    tokens: 500000,
    price: 4,
    subtitle: "good for X–Y messages on GPT-4o-mini",
    badge: null,
    saveText: null,
  },
  {
    ...
    >
      <header className={styles.topUpHeader}>
        ...
        <div className={styles.textInfoContainer}>
          ...
          <p className={styles.fullparagraph}>
            No subscriptions. Tokens never expire. Use them anytime.
          </p>
          <p className={styles.notFullparagraph}>
      ...
      <section className={styles.plansContainer}>
        ...
        <article className={styles.plansCard1}>
          ...
          <h2 className={styles.tokensTitle}>
            {PLANS[0].tokens.toLocaleString("en-US")} tokens
          </h2>
          <p className={styles.plansParagraph2}>{PLANS[0].subtitle}</p>
          <h2 className={styles.priceTitle}>${PLANS[0].price}</h2>

          <button
            className={styles.buyButton}
            onClick={() => openPayment(PLANS[0])}
            type="button"
          >
            <div className={styles.buyButtonContent}>
              <span className={styles.buyButtonSpan}>Buy tokens</span>
              <Image
                className={styles.buyButtonIcon}
                width={26}
                height={26}
        ...
        <article className={styles.plansCard2}>
          ...
          <h2 className={styles.tokensTitle}>
            {PLANS[1].tokens.toLocaleString("en-US")} tokens
          </h2>
          <p
            className={`${styles.plansParagraph2} ${styles.addToPlansParagraph2}`}
          >
            best for daily usage
          </p>
          <h2 className={styles.priceTitle}>
            <span className={styles.priceTitleSpan}>${PLANS[1].price}</span>
            <span className={styles.saveSpan1}>Save 25%</span>
          </h2>

          <button
            className={styles.buyButton2}
            onClick={() => openPayment(PLANS[1])}
            type="button"
          >
            <div className={styles.buyButtonContent}>
              <span className={styles.buyButtonSpan}>Buy tokens</span>
        ...
        <article className={styles.plansCard3}>
          ...
          <p className={styles.plansParagraph1}>PACKAGE</p>
          <h2 className={styles.tokensTitle}>
            {PLANS[2].tokens.toLocaleString("en-US")} tokens
          </h2>
          <p className={styles.plansParagraph2}>best for power users</p>
          <h2 className={styles.priceTitle}>
            <span>${PLANS[2].price}</span>
            <span className={styles.saveSpan2}>Save 50%</span>
          </h2>

          <button
            className={styles.buyButton3}
            onClick={() => openPayment(PLANS[2])}
            type="button"
          >
            <div className={styles.buyButtonContent}>
              <span className={styles.buyButtonSpan}>Buy tokens</span>
      ...
      <section className={styles.freeTokens}>
        <div className={styles.freeTokensInfo}>
          ...
          <span className={styles.freeTokensSpan}>
            Claim 10,000 tokens once per week.
          </span>
          <span className={styles.freeTokensSpan2}>
            Make the most of premium models — on your schedule.
          </span>

          <p className={styles.presentParagraph}>
            <span className={styles.freeTokensSpan3}>
              NEXT CLAIM IN:
              <span className={styles.freeTokensSpan4}>{countdown}</span>
            </span>
          </p>
        </div>
      ...
      <footer className={styles.footer}>
        <span className={styles.footerSpan}>
          * We show an approximate request cost based on a typical 30-word
          message. Actual token usage depends on how much you write.
        </span>

        <span className={styles.adaptiveFooterSpan}>
          * Approximate request price is based on a typical 30-word message.
        </span>

        <span className={styles.footerSpan}>
          * Different models may consume tokens at different rates. Your balance
        ...
        <span className={styles.gptitiRights}>
